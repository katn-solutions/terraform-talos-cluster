...
variable "kafka_vpc_access_cidr" {
  type        = list(string)
  description = "CIDR block for VPC access to Kafka"
}
...
variable "kafka_port_start" {
  type        = number
  description = "Beginning port for kafka traffic"
}

variable "kafka_port_end" {
  type        = number
  description = "End port for kafka traffic"
}
...
# Allow Kafka ports from within the VPCs
resource "aws_security_group_rule" "kafka" {
  count             = length(var.kafka_vpc_access_cidr) > 0 ? 1 : 0
  type              = "ingress"
  from_port         = var.kafka_port_start
  to_port           = var.kafka_port_end
  protocol          = "TCP"
  security_group_id = aws_security_group.cluster-node.id
  cidr_blocks       = var.kafka_vpc_access_cidr
}
...
# Allow Traffic from Kafka Internal Load Balancer
resource "aws_security_group_rule" "ingress-kafka-int" {
  type                     = "ingress"
  from_port                = 31999
  to_port                  = 32002
  protocol                 = "TCP"
...
# Allow Traffic from Kafka External Load Balancer
resource "aws_security_group_rule" "ingress-kafka-ext" {
  type                     = "ingress"
  from_port                = 31999
  to_port                  = 32002
  protocol                 = "TCP"
